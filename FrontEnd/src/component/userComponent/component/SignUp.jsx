  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (values.confPassword === values.password) {
        ...
        axios({ method: "POST", url: "http://localhost:5000/sendOTP/otp", data: { email: values.email } });
        Swal.fire({
          title: "Enter OTP",
          input: "text",
          inputLabel: "",
          ...
          preConfirm: (otp) => {
            return axios
              .post("http://localhost:5000/sendOTP/verifyOTP", { values,otp })
              .then((response) => {
          ...
          if (result.value) {
            ...
            axios({ method: "POST", url: "http://localhost:5000/customer/add", data: { name: values.name,appartmentNo: values.apartmentNo,email: values.email,phoneNo: values.phoneNo,nicNo: values.nicNo,password: values.password } }).then(()=>{
